...
export default function AddExerciseModal({ open, onClose, onAdd }: Props) {
  ...
  // states for filtering muscle and exercise type
  ...
  const hasActiveFilter = Boolean(search || muscle || type);
            ...
            <div className="rounded-3xl border border-zinc-800 bg-[#090d16] p-4">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <select
                  ...
                  className="w-full rounded-2xl border border-zinc-800 bg-black/30 px-4 py-3 text-sm text-white outline-none transition focus:border-[#85ADFF]"
                >
                  <option value="" className="bg-[#090d16] text-white">
                    All muscles
                  </option>
                  <option value="chest" className="bg-[#090d16] text-white">
                    Chest
                  </option>
                  <option value="back" className="bg-[#090d16] text-white">
                    Back
                  </option>
                  <option value="legs" className="bg-[#090d16] text-white">
                    Legs
                  </option>
                  <option value="shoulders" className="bg-[#090d16] text-white">
                    Shoulders
                  </option>
                  <option value="biceps" className="bg-[#090d16] text-white">
                    Biceps
                  </option>
                  <option value="triceps" className="bg-[#090d16] text-white">
                    Triceps
                  </option>
                </select>
                ...
                >
                  ...
                  <option value="compound" className="bg-[#090d16] text-white">
                    Compound
                  </option>
                  <option value="isolation" className="bg-[#090d16] text-white">
                    Isolation
                  </option>
                  <option
                    value="bodyweight"
                ...
                {!hasActiveFilter && (
                  <div className="flex h-full items-center justify-center rounded-2xl border border-dashed border-zinc-800 text-center">
                    <p className="max-w-xs text-sm text-zinc-500">
                      Start typing or choose a filter to find exercises.
                    </p>
                  </div>
                )}

                {hasActiveFilter && filteredExercises.length === 0 && (
                  <div className="flex h-full items-center justify-center rounded-2xl border border-dashed border-zinc-800 text-center">
                    <p className="text-sm text-zinc-500">No exercises found.</p>
                  </div>
                )}

                {/* When filters or search we map through exercises and display.*/}
